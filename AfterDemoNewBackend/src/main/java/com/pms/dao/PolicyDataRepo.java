...
public interface PolicyDataRepo  extends JpaRepository<PolicyTable, String>  {
    ...
    @Query(value="SELECT pt.policy_type FROM user_enrolled_policy uep \n" + 
    		"join policy_table pt on uep.policy_policy_id = pt.policy_id where uep.user_user_id=:userid \n" + 
    		"  group by  uep.user_user_id,pt.policy_type having count(pt.policy_type) =2;",nativeQuery=true)
