...
public class ServiceImpl implements Service1{
		...
		if(name.size() == 0) {
			...
			{   System.out.println(user.getPanNo());
				if(user.getPanNo().matches(("^[A-Za-z]{3}["+user.getFirstName().toUpperCase().charAt(0)+"]["+user.getLastName().toUpperCase().charAt(0)+"][0-9]{4}["+dat1.getMonth().getDisplayName(TextStyle.FULL, Locale.UK).charAt(0)+"]$")))
				{
					...
					mess="The user id is "+user.getUserid()+" and password **"+user.getPasscode().substring(2,5)+"***";
	...
	//New
	@Override
	public List<SearchReturn> searchUserPolicy(String companyId, int duration, String policyType, String policyName,String userid) {
	...
	@Override
	public List<SearchReturn> testuse1(String userid) {
	
		List<String> a=dao2.searchUserPolicy1(userid);
		List<PolicyTable> e=new ArrayList<>();
		e=dao2.searchUserPolicy2(a);
		List<SearchReturn> sr=new ArrayList<>();
		for(int i=0;i<e.size();i++) {
			SearchReturn ssr =new SearchReturn();
			ssr.setPolicyId(e.get(i).getPolicyId());
	...
	@Override
	public List<SearchReturn> testuse2(String userid) {
		
		Mainuser a1= dao1.findByUserid(userid);
        Map<String,Integer> s=new HashMap<>();
        s.put("Health", a1.getHealth());
        s.put("Child", a1.getChild());
        s.put("Retirement", a1.getRetirement());
        s.put("Travel",a1.getTravel());
        s.put("Life", a1.getLife());
        s.put("Vehicle", a1.getVehicle());
        String key = Collections.max(s.entrySet(), Map.Entry.comparingByValue()).getKey();
    	System.out.println(key);
    	List<String> a=dao2.searchUserPolicy1(userid);
		List<PolicyTable> e=new ArrayList<>();
		e=dao2.searchUserPolicy2(a);
		if(!a.contains(key)) {
		  e.addAll(dao2.findByPolicyType(key));
